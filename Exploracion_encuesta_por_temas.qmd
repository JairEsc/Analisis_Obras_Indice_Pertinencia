...
-   Cuál.considera.que.es.el.principal.problema.que.enfrenta.el.estado.de.Hidalgo. (Servicios Públicos)
...
-   Y...cuál.considera.que.es.el.principal.problema.en.su.colonia.(Servicios públicos)
...
z4[abs(z4)<1e-5]=NA
z5="Inputs/Rasters_Generados_en_R/Otros/exploracion_encuesta/Por temas/Carreteras/infraestructura_percepcion_negativa_carreteras.tif" |> raster()
z5[abs(z5)<1e-4]=NA
z5=-z5
values(z5)[1]=maxValue(abs(z5))
z6="Inputs/Rasters_Generados_en_R/Otros/exploracion_encuesta/Por temas/Carreteras/infraestructura_percepcion_negativa_vialidades.tif" |> raster()
z6[abs(z6)<1e-4]=NA
...
leaflet() |> addTiles()|> 
  addRasterImage(z,group='Infraestructura Educativa') |>
  addRasterImage(z2,group='Salud') |>
  addRasterImage(z3,group='Espacios Públicos') |>
  addRasterImage(z4,group='Agua y servicios públicos') |>
  addRasterImage(z5,group='Infraestructura Vial (carreteras)') |>
  addRasterImage(z6,group='Infraestructura Vial (calles y vialidades)') |>
  
  addLegendNumeric( pal = colorNumeric('Spectral', 1:100) , values = 1:100, position = 'bottomright', title = 'Escala percepción (negativa - > positiva)', orientation = 'horizontal', shape = 'rect', decreasing = FALSE, height = 20,width = 300,labels = '',tickLength = 0) |> addLayersControl(overlayGroups = c('Infraestructura Educativa','Salud','Espacios Públicos','Agua y servicios públicos','Infraestructura Vial (carreteras)','Infraestructura Vial (calles y vialidades)'
     ),options = layersControlOptions(collapsed = F)) |> hideGroup(c('Salud','Espacios Públicos','Agua y servicios públicos','Infraestructura Vial (carreteras)','Infraestructura Vial (calles y vialidades)'
     )) 
```
...
colnames(rasters_by_cells)=c('Infraestructura Educativa','Salud','Espacios Públicos','Agua y servicios públicos','Infraestructura Vial (carreteras)','Infraestructura Vial (calles y vialidades)'
     )
