...
leaflet() |> addTiles()|> 
  ...
  addRasterImage(z4,group='Agua (suministro)') |>
  addRasterImage(z5,group='Drenaje') |>
  addRasterImage(z6,group='Infraestructura Vial') |>
  
  addLegendNumeric( pal = colorNumeric('Spectral', 1:100) , values = 1:100, position = 'bottomright', title = 'Escala percepción (negativa - > positiva)', orientation = 'horizontal', shape = 'rect', decreasing = FALSE, height = 20,width = 300,labels = '',tickLength = 0) |> addLayersControl(overlayGroups = c('Infraestructura Educativa','Salud','Espacios Públicos','Agua (suministro)','Drenaje','Infraestructura Vial'
     ),options = layersControlOptions(collapsed = F)) |> hideGroup(c('Salud','Espacios Públicos','Agua (suministro)','Drenaje','Infraestructura Vial'
     )) 
```
