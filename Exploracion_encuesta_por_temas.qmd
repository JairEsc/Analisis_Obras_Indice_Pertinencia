...
regionalizacion=st_read(buig,"regionalizacion")
lims_mun=st_read(buig,"limite_municipal")
lims_mun=lims_mun |> merge(y=regionalizacion |> dplyr::select(nom_mun,region) |> st_drop_geometry(),by.x='nomgeo',by.y='nom_mun',all.x=T)



leaflet() |> addTiles()|> 
  addPolygons(data=lims_mun |> 
  dplyr::group_by(region) |> 
  dplyr::summarise(the_geom=st_union(the_geom)) ,fillColor = "#D4C19C",fillOpacity = 0.35,color = "white",opacity = 0.9,stroke = 2,group = 'Regionalizacion') |> 
